...
#  por ejemplo al Abrir el menú o hablar con un npc.
  ...
  def initialize(name)
    ...
    # Añil: Poké Ball en el letrero según si ya capturaste en esta zona (gris = te falta,
    # color = ya capturaste). Solo en zonas de captura con una regla de captura activa.
    @anil_ball = nil
    if defined?(ChallengeModes) && ChallengeModes.respond_to?(:current_zone_trackable?) && ChallengeModes.current_zone_trackable?
      cap = (ChallengeModes.zone_captured? rescue false)
      icon = cap ? "icon_own" : "icon_own_gray"
      bmp = (RPG::Cache.load_bitmap("Graphics/Pictures/DP Pause Menu/", icon) rescue nil)
      if bmp
        @anil_ball = Sprite.new(vista)
        @anil_ball.bitmap = bmp
        @anil_ball.z = 100000
        @anil_ball.visible = false   # update la posiciona y la muestra (evita parpadeo en 0,0)
      end
    end
  end
  ...
  def dispose
    ...
    @w2.dispose   if @w2
    @anil_ball.dispose if @anil_ball && !@anil_ball.disposed?
  end

  def anil_ball_dispose
    @anil_ball.dispose if @anil_ball && !@anil_ball.disposed?
    @anil_ball = nil
  end
  ...
  def update
    ...
    # Añil: la Poké Ball se pega a la izquierda del texto y se centra vertical con la ventana.
    # Se elimina cuando el texto se vacía (fin del letrero; en "LetsGO" la ventana no se dispone
    # nunca por un elsif muerto del plugin, así que nos guiamos por el texto) o al disponerse.
    if @anil_ball && !@anil_ball.disposed?
      if @window.text.to_s.empty?
        anil_ball_dispose
      else
        bh = (@anil_ball.bitmap ? @anil_ball.bitmap.height : 28)
        @anil_ball.x = @window.x - 22
        @anil_ball.y = @window.y + [(@window.height - bh) / 2, 0].max
        @anil_ball.visible = @window.visible
      end
    end
    @window.update
    @pic.update
