...
class PokemonSummary_Scene
  ...
  def drawPageTwo
    ...
    # Nuzlocke EX: marca "Repetido" (prioritaria) o "Captura EXTRA".
    anil_dup = (defined?(pbAnilDuplicate?) && pbAnilDuplicate?(@pokemon)) rescue false
    anil_extra = (@pokemon.respond_to?(:anil_extra_capture) && @pokemon.anil_extra_capture)
    if anil_dup || anil_extra
      encounter_memo += "\n" if !encounter_memo.empty? && !encounter_memo.end_with?("\n")
      encounter_memo += blue_text_tag + (anil_dup ? _INTL("Repetido.") : _INTL("Captura EXTRA."))
    end
    # Añil: marca de ORIGEN (Primer de ruta / Fósil / Intercambio NPC / Don Prodigio / Estático...).
    anil_origin = (defined?(AnilOrigin) ? AnilOrigin.label(@pokemon) : nil) rescue nil
    if anil_origin
      encounter_memo += "\n" if !encounter_memo.empty? && !encounter_memo.end_with?("\n")
      encounter_memo += blue_text_tag + _INTL("Origen: {1}.", anil_origin)
    end
    # Write all text
