...
def pbOnStepTaken(eventTriggered)
  ...
  if !eventTriggered && !$game_temp.in_menu
    if $PokemonSystem.salvajes_visibles_en_ow == 1 && $game_variables[55] >= 5 #pbBattleOrSpawnOnStepTaken(repel_active)
      pbBattleOnStepTaken(repel_active) # STANDARD WILD BATTLE
    else
      pbSpawnOnStepTaken(repel_active)  # OVERWORLD ENCOUNTERS
    end
...
def voe_enabled?
  return true if $PokemonSystem.salvajes_visibles_en_ow == 0 || $game_variables[55] < 5
...
def pbSpawnOnStepTaken(repel_active)
  return if $PokemonSystem.salvajes_visibles_en_ow == 1 || $game_variables[55] < 5 # Solo spawnean cuando está a true.
  ...
  if $PokemonEncounters.allow_encounter?(encounter, repel_active)
    ...
    encounter_class = $PokemonEncounters.encounter_class_on_tile(pos[0],pos[1])
    # Terreno de hierba
    if encounter_class == "Water"
      #echoln "Animación de encuentro en agua"
      $scene.spriteset.addUserAnimation(23,pos[0],pos[1])
    # Encuentro hierba
    elsif encounter_class == "Land" || encounter_class == "LandClassic"
      #echoln "Animación de encuentro en hierba"
      if $game_map && $game_map.map_id==174 # Animación de hierba morada
        $scene.spriteset.addUserAnimation(24,pos[0],pos[1])
      else
        $scene.spriteset.addUserAnimation(Settings::RUSTLE_NORMAL_ANIMATION_ID,pos[0],pos[1])
      end
    else # Cuevas y demás
      #echoln "Animación de encuentro en cueva"
      $scene.spriteset.addUserAnimation(2,pos[0],pos[1])
    end
  end
