...
class DP_PauseMenu
  ...
  def draw_vial_shortcut(refresh = false)
      ...
      @sprites[:vial_txt].draw([
                            ["[A] Vial (#{$PokemonGlobal.vial_charges}/#{$PokemonGlobal.max_vial_charges})", TEXT_X, @vial_y_pos + TEXT_Y_OFFSET, 0, base_color, shadow_color, true]
                          ])
  end
  ...
  def draw_helpful_text
    ...
    @sprites[:helpful_text_txt].draw([
        ...
        [_INTL('[F1] Controles'), Graphics.width - 155, Graphics.height - 30, 0, base_color, shadow_color, true]
    ])
  ...
  def draw_captured_icon
    cc = ChallengeModes.on?(:CAPTURE_COUNTER)
    return if !ChallengeModes.on?(:ONE_CAPTURE) && !cc
    # Solo se muestra en zonas donde SE PUEDE capturar (con encuentros). En centros
    # Pokémon, tiendas, interiores sin hierba, etc. no aparece nada.
    return if ChallengeModes.respond_to?(:current_zone_trackable?) && !ChallengeModes.current_zone_trackable?
    captured = ChallengeModes.zone_captured?
    # ONE_CAPTURE: comportamiento vanilla (solo se muestra si ya capturaste).
    # CAPTURE_COUNTER: siempre visible con estado -> gris = zona libre, color = ya capturaste.
    return if !captured && !cc
    y_pos = get_next_right_icon_y_position
    ...
    icon_name = (cc && !captured) ? "icon_own_gray" : "icon_own"
    bitmap = pbBitmap(File.join(ICONS_PATH, icon_name))
