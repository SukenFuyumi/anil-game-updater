...
class CableClubScreen
  ...
  def pbAttemptConnection
    ...
    loop do
      begin
        ...
        regex_check = /[0-9]/
        loop do
          # Código flexible: cualquier número (1 a GROUP_ID_LENGTH dígitos). Emparejan si coinciden.
          pbMessage("Pon el código de la sala: un número cualquiera (de 1 hasta #{CableClub::GROUP_ID_LENGTH} dígitos). La otra persona debe poner el MISMO código para emparejar.")
          ...
          break if group_id =~ /^[0-9]{1,#{CableClub::GROUP_ID_LENGTH}}$/
        end
        group_id = group_id.to_i   # se envía y compara como número (así "7" y "07" emparejan)
  ...
  def await_partner(connection,group_id)
    ...
    change_state(:await_partner){  
      ...
      connection.update do |record|
        ...
        when :found
          pbAnilLog("FOUND: recibido, parseando...") rescue nil
          ...
          pbAnilLog("FOUND: cabecera ok (id=#{@client_id} name=#{@partner_name}); parseando party...") rescue nil
          @partner_party = CableClub::parse_party(record)
          pbAnilLog("FOUND: party ok (#{(@partner_party||[]).length} pkmn); parseando rules...") rescue nil
          @server_rules = CableClub::parse_battle_rules(record)
          pbAnilLog("FOUND: rules ok (#{(@server_rules||[]).length}); record vacio?=#{record.empty?}") rescue nil
          ...
          partner_found = true
          pbAnilLog("FOUND ok final: client_id=#{@client_id}") rescue nil
        else
          echoln "Unknown message: #{type}"
          pbAnilLog("await_partner mensaje inesperado: #{type}") rescue nil
        end
      end
    ...
    if partner_found
      ...
      pbAnilLog("rama: #{@client_id == 0 ? 'choose_activity (yo elijo)' : 'await_choose_activity (espero)'}") rescue nil
      if @client_id == 0
        choose_activity(connection)
      else
        await_choose_activity(connection)
      end
      pbAnilLog("volví de la rama de actividad (se cierra la sesión)") rescue nil
    end
  end
  
  def choose_activity(connection)
    pbAnilLog("choose_activity: entré (restore_original_team=#{$original_player_party ? 'SI' : 'no'})") rescue nil
    ...
    # Construimos el menú junto a su símbolo de actividad, para no depender de índices fijos
    # (así añadir/quitar opciones no rompe el despacho).
    cmds = []
    acts = []
    cmds.push(_INTL("Combate"));                acts.push(:battle)
    cmds.push(_INTL("Intercambio"));            acts.push(:trade)
    cmds.push(_INTL("Intercambio de objetos")); acts.push(:item_trade)
    if CableClub::ENABLE_RECORD_MIXER
      cmds.push(_INTL("Mix Records")); acts.push(:record_mix)
    end
    cmds.push(_INTL("Salir")); acts.push(:exit)
    cmd = -1
    pbAnilLog("choose_activity: mostrando menú de actividad") rescue nil
    change_state(:choose_activity){
      cmd = pbShowCommands(_INTL("Elige una actividad."), cmds, -1)
      break
    }
    chosen = (cmd >= 0 && acts[cmd]) ? acts[cmd] : :exit
    pbAnilLog("choose_activity: elegí #{chosen}") rescue nil
    case chosen
    when :battle
      @battle_settings = nil
    ...
    when :item_trade
      connection.send do |writer|
        writer.sym(:item_trade)
      end
      await_accept_activity(connection,:item_trade,:choose_trade_item)
    when :record_mix
      connection.send do |writer|
  ...
  def await_choose_activity(connection)
    ...
    change_state(:await_choose_activity){
      ...
      connection.update do |record|
        ...
        when :item_trade
          method_for_accepting = :partner_accept_item_trade
        when :record_mix
          method_for_accepting = :partner_accept_record_mix
        when :noop
          # No hace nada
        when :ok
          # :ok+orden rezagado de una selección de equipo cancelada (llega tarde por latencia).
          # Lo consumimos por completo para no romper el protocolo (evita el "gracias" por
          # campos sin leer) y seguimos esperando.
          n = (record.int rescue 0).to_i
          n.times { record.int rescue nil }
        when :cancel
          # Cancelación rezagada del compañero: la ignoramos y seguimos esperando.
        else
          echoln "Unknown message: #{type}"
        end
      end
  ...
  # Tras CANCELAR, drena el buffer para consumir (leyendo bien sus campos) cualquier mensaje
  # que el compañero ya haya enviado (p.ej. su ":ok"+orden). Así no queda basura en el buffer
  # que rompa el reenvío (evita el desync de "un jugador espera" y el error que da "gracias").
  def anil_drain_cancel(connection)
    frames = 0
    done = false
    while frames < 20 && !done
      begin
        connection.update do |record|
          t = record.sym
          if t == :ok
            n = (record.int rescue 0).to_i
            n.times { record.int rescue nil }   # consume la longitud + el orden del equipo
            done = true                          # ese era el mensaje pendiente; listo
          end
          # :cancel u otros: el símbolo ya se consumió; se ignora
        end
      rescue
        # ignora cualquier error de protocolo mientras drenamos
      end
      Graphics.update
      Input.update
      frames += 1
  ...
  def choose_battle_settings(connection)
    ...
    if battle_rule
      ...
      anil_drain_cancel(connection)   # consume mensajes pendientes del compañero antes de volver
      if @client_id == 0
        choose_activity(connection)
      else
        await_choose_activity(connection)
  ...
  def battle_check_team_preview(connection)
    ...
    if team_order
      ...
      anil_drain_cancel(connection)   # consume el :ok+orden que el compañero ya pudo enviar
      cancel_battle = true
      if @client_id == 0
        choose_activity(connection)
      else
    ...
    if cancel_battle || cancel_partner
      ...
      pbAnilLog("órdenes OK (mio=#{(team_order||[]).length} partner=#{(partner_order||[]).length}); entrando a do_battle") rescue nil
      do_battle(connection, team_order, partner_order)
      pbAnilLog("do_battle retornó") rescue nil
      exchange_teams(connection) if restore_original_team
    end
  end
  ...
  def do_battle(connection,team_order,partner_order)
    ...
    pbAnilLog("do_battle: mi_equipo=#{party_player.length} partner_equipo=#{party_partner.length} seed=#{seed}; llamando CableClub.do_battle") rescue nil
    decision = CableClub::do_battle(connection, @client_id, seed, battle_rules[2], party_player, partner, party_partner)
    pbAnilLog("CableClub.do_battle terminó (decision=#{decision})") rescue nil
    # Recuperamos posible equipo original
    exchange_teams(connection) if restore_original_team
  ...
  def choose_trade_pokemon(connection)
    # Añil: antes de intercambiar, devolvemos los EV/IV ORIGINALES al equipo. Los combates
    # online usan IVs=31 + EVs temporales del editor; un Pokémon NO debe intercambiarse con
    # esos stats prestados (sería un buff injusto permanente para el otro jugador y dejaría
    # el respaldo inconsistente). Este método lo ejecutan AMBOS lados antes de elegir Pokémon.
    AnilOnlineEV.restore_party if defined?(AnilOnlineEV)
  ...
  #=============================================================================
  # Intercambio de OBJETOS (cliente↔cliente por el relay; el servidor no valida).
  # Cada lado elige un objeto (+cantidad) de su mochila, se muestran ambas ofertas,
  # ambos confirman y se aplica localmente (cada uno sabe lo que da y lo que recibe).
  # Se excluyen los objetos clave.
  #=============================================================================
  def partner_accept_item_trade(connection)
    if pbConfirm(_INTL("¡{1} quiere intercambiar objetos!", @partner_name))
      connection.send do |writer|
        writer.sym(:ok)
      end
      choose_trade_item(connection)
    else
      connection.send do |writer|
        writer.sym(:cancel)
      end
  ...
  # Abre la mochila y devuelve [item_id, cantidad] o [nil, 0] si se cancela.
  def anil_choose_bag_item
    item = nil
    pbFadeOutIn(99999) {
      scene  = PokemonBag_Scene.new
      screen = PokemonBagScreen.new(scene, $bag)
      # No se pueden ofrecer objetos clave.
      item = screen.pbChooseItemScreen(Proc.new { |itm| !GameData::Item.get(itm).is_key_item? })
    }
    return [nil, 0] if !item
    qty_have = $bag.quantity(item)
    amt = 1
    if qty_have > 1
      params = ChooseNumberParams.new
      ...
      amt = pbMessageChooseNumber(
        _INTL("¿Cuántos {1} quieres ofrecer? (tienes {2})", GameData::Item.get(item).name, qty_have), params)
    end
    return [nil, 0] if amt.nil? || amt <= 0
    return [item, amt]
  end

  def choose_trade_item(connection)
    pbHideMessageBox
    # Reutilizamos la sala del intercambio (switch 134).
    CableClub.transfer_to_trade_room if !$game_switches[134]
    item, qty = anil_choose_bag_item
    if item
      @my_item = item
      @my_qty  = qty
      connection.send do |writer|
  ...
  def confirm_trade_item(connection)
    @partner_item = nil
    @partner_qty  = nil
    partner_cancelled = false
    change_state(:await_item_trade){
      pbDisplayDots(_INTL("Esperando a que {1}\nelija un objeto", @partner_name))
      connection.update do |record|
        case (type = record.sym)
        when :ok
          @partner_item = record.sym
          @partner_qty  = record.int
        when :cancel
      ...
      break if !@partner_item.nil? || partner_cancelled
    }
    trade_state = :waiting
    if !partner_cancelled && @partner_item
      my_name      = GameData::Item.get(@my_item).name
      partner_name = GameData::Item.get(@partner_item).name
      loop do
        cmd = pbShowCommands(
          _INTL("{1} ofrece {2} x{3} por tu {4} x{5}.",
                @partner_name, partner_name, @partner_qty, my_name, @my_qty),
          [_INTL("Aceptar"), _INTL("Rechazar")], -1)
        case cmd
        when 0 # Aceptar
          trade_state = :ok
          connection.send do |writer|
          ...
          break
        else # Rechazar / cancelar
          trade_state = :denied
          connection.send do |writer|
            writer.sym(:cancel)
          end
          connection.discard(1)
          pbMessage(_INTL("Has cancelado el intercambio.")) if cmd < 0
          break
        end
      end
      await_item_trade_partner(connection) if trade_state == :ok
    else
      trade_state = :cancel
    end
    if trade_state != :ok
      @my_item = nil; @my_qty = nil; @partner_item = nil; @partner_qty = nil
    end
    case trade_state
    when :cancel; pbDisplay(_INTL("Lo siento, {1} no quiere intercambiar.", @partner_name))
    end
    return if trade_state == :ok  # do_item_trade se encarga del retorno al menú
    if @client_id == 0
      choose_activity(connection)
    else
      await_choose_activity(connection)
  ...
  def await_item_trade_partner(connection)
    partner_confirm = nil
    change_state(:confirm_item_trade){
      pbDisplayDots(_INTL("Esperando a que {1}\nconfirme el intercambio", @partner_name))
      connection.update do |record|
        case (type = record.sym)
        when :ok
  ...
  def do_item_trade(connection)
    # Ambos lados aplican a la vez: quito lo que ofrecí, recibo lo del compañero.
    got_item = @partner_item
    got_qty  = @partner_qty
    $bag.remove(@my_item, @my_qty)
    $bag.add(got_item, got_qty)
    pbSEPlay("Mart buy item") rescue nil
    pbDisplay(_INTL("¡Has recibido {1} x{2} de {3}!",
                    GameData::Item.get(got_item).name, got_qty, @partner_name))
    @my_item = nil; @my_qty = nil; @partner_item = nil; @partner_qty = nil
    if @client_id == 0
      choose_activity(connection)
    else
