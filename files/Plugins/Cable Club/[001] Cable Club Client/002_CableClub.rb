...
module CableClub
  ...
  ACTIVITY_OPTIONS = {:battle => _INTL("Combate"),
                      ...
                      :item_trade => _INTL("Intercambio de objetos"),
  ...
  def self.do_battle(connection, client_id, seed, battle_rules, player_party, partner, partner_party)
    pbAnilLog("CableClub.do_battle: INICIO (client_id=#{client_id})") rescue nil
    ...
    exc = nil
    outcome = 0
    # Marca el combate como online: mientras esté activo, un Pokémon debilitado NO
    # queda con muerte permanente (ver 004_Permafaint.rb). Así el curado posterior
    # lo restaura y no aparece muerto en el siguiente combate ni en el mapa.
    $anil_online_battle_active = true

    # Saltamos al mapa del combate.
    pbAnilLog("do_battle: antes de transfer (switch_combate=#{$game_switches[SWITCH_ONLINE_COMBATE]})") rescue nil
    self.transfer_to_battle_room if !$game_switches[SWITCH_ONLINE_COMBATE] # Solo vamos a la sala online si aún no estamos ahí.
    pbAnilLog("do_battle: después de transfer, antes de pbBattleAnimation") rescue nil

    pbBattleAnimation(nil, (battle.singleBattle?) ? 1 : 3, [partner]) {
      pbSceneStandby {
        begin
          pbAnilLog("do_battle: llamando pbStartBattle") rescue nil
          outcome = battle.pbStartBattle
          pbAnilLog("do_battle: pbStartBattle terminó outcome=#{outcome}") rescue nil
        rescue Connection::Disconnected
          scene.pbEndBattle(0)
          exc = $!
        ensure
          $anil_online_battle_active = false
  ...
  def self.transfer_to_battle_room
    ...
    x = 50
    y = 25
    pbAnilLog("transfer: antes de pbFadeOutIn/transfer_player") rescue nil
    pbFadeOutIn(99999){
    ...
    pbAnilLog("transfer: mapa 218 cargado, antes de pasos") rescue nil
    pbWait(1)
    # Hacer que el jugador dé tres pasos hacia adelante. (Con tope de frames por si la
    # animación no avanza —p.ej. una ventana sin foco— para no quedar en bucle infinito.)
    3.times { $game_player.move_up
      guard = 0
      while $game_player.moving? && guard < 40
        Graphics.update
        Input.update
        pbUpdateSceneMap
        guard += 1
    ...
    pbAnilLog("transfer: pasos dados, antes de scroll") rescue nil
    pbWait(1)
    ...
    loop do
      ...
      pbUpdateSceneMap
      guard += 1
      break if !$game_map.scrolling? || guard > 90
    end
    pbAnilLog("transfer: scroll done, antes del mensaje") rescue nil
    pbWait(2)
    pbMessage(_INTL("¡Comienza el combate contra {1}! ¡Adelante!",$partner_online[0]))
    pbAnilLog("transfer: mensaje mostrado (FIN)") rescue nil
    pbWait(1)
    # pbCameraReset
