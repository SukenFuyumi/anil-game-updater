...
module ChallengeModes
  ...
  def set_first_encounter(pkmn, owned_flag = nil)
    return if !ChallengeModes.on?(:FIRST_CAPTURE) && !ChallengeModes.on?(:ONE_CAPTURE) && !ChallengeModes.on?(:CAPTURE_COUNTER)
    ...
    # Contador de Capturas: los shiny son siempre captura extra asegurada -> no consumen la zona.
    captured = false if ChallengeModes.on?(:CAPTURE_COUNTER) && pkmn.shiny?
    # Contador de Capturas: un REPETIDO (familia ya poseída ANTES de este encuentro) tampoco
    # consume la zona -> puedes seguir buscando el primero legítimo de la ruta. Usamos owned_flag
    # (estado previo a la captura) para no contar como "poseído" al que se acaba de atrapar.
    if ChallengeModes.on?(:CAPTURE_COUNTER) && captured
      sp_data.get_family_species.each do |pk|
        captured = false if (owned_flag.nil? ? $player.owned?(pk) : owned_flag)
      end
    end
  ...
  def had_first_encounter?(pkmn = nil)
    return false if !ChallengeModes.on?(:FIRST_CAPTURE) && !ChallengeModes.on?(:ONE_CAPTURE) && !ChallengeModes.on?(:CAPTURE_COUNTER)
    return false if pkmn && pkmn.shiny? && (ChallengeModes.on?(:SHINY_CLAUSE) || ChallengeModes.on?(:CAPTURE_COUNTER))
