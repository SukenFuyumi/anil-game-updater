...
class Battle::Scene
  ...
  def should_set_first_encounter?(battler)
    ...
    return false if (!ChallengeModes.on?(:FIRST_CAPTURE) && !ChallengeModes.on?(:CAPTURE_COUNTER)) || ChallengeModes.had_first_encounter?
    return false if defined?(MonotypeChallenge) && MonotypeChallenge.enabled? && !MonotypeChallenge.valid_monotype?(battler)
    # Legendarios/singulares no cuentan al debilitarlos/huir (con Cláusula Legendarios o Contador de Capturas).
    # Paradojas/Ultraentes NO llevan estos flags -> sí cuentan.
    return false if legendary_flags.any? { |flag| battler.pokemon.species_data.has_flag?(flag) } && (ChallengeModes.on?(:LEGENDARY_CLAUSE) || ChallengeModes.on?(:CAPTURE_COUNTER))
    return true
  end
...
class Battle
  ...
  def pbThrowPokeBall(*args)
    ...
    owned_b4 = false
    # ¿La zona ya tenía captura registrada ANTES de este lanzamiento? (para "Primer de ruta")
    zone_had_capture = (ChallengeModes.on? ? ChallengeModes.had_first_encounter? : true) rescue true
    ...
    # Contador de Capturas: marcar como EXTRA si el jugador confirmó la captura extra en este
    # combate, o si es shiny (siempre captura extra asegurada). Se avisa al momento.
    if @caughtPokemon.length != caught
      last = @caughtPokemon.last
      if last && last.respond_to?(:anil_extra_capture=)
        if anil_extra_pending || (ChallengeModes.on?(:CAPTURE_COUNTER) && last.shiny?)
          last.anil_extra_capture = true
          pbMessage(_INTL("¡{1} queda registrado como captura EXTRA!\nPodrás verlo en su resumen (pág. Encuentro).", last.name)) if ChallengeModes.on?(:CAPTURE_COUNTER)
        elsif !zone_had_capture && last.respond_to?(:anil_first_route=) &&
              !((defined?(pbAnilDuplicate?) && pbAnilDuplicate?(last)) rescue false)
          # Captura legítima en una zona que aún no tenía captura y que NO es repetido:
          # es el "Primer de ruta". Un repetido no debe contar como el primero de la ruta.
          last.anil_first_route = true
        end
      end
    end
...
ItemHandlers::CanUseInBattle.addIf(:poke_balls,
  ...
  proc { |item, pokemon, battler, move, firstAction, battle, scene, showMessages|
    ...
    # Ya hubo captura en esta zona
    if !battle.trainerBattle? && ChallengeModes.had_first_encounter?(target) #&& !ChallengeModes.on?(:MONOTYPE_CLAUSE)
      if ChallengeModes.counter_only?
        # CONTADOR DE CAPTURAS: aviso suave, deja continuar (para capturas extra / estáticos).
        # Solo se confirma UNA vez por combate; si la Poké Ball falla no vuelve a preguntar.
        if showMessages && !battle.anil_extra_pending
          pbSEStop
          if !pbConfirmMessage(_INTL("Ya capturaste en esta zona. Solo continúa si tienes una captura extra o es un Pokémon estático.\n¿Lanzar de todos modos?"))
            next false
          end
          battle.anil_extra_pending = true   # confirmado: las capturas de este combate serán extra
        end
      else
        # FIRST_CAPTURE / ONE_CAPTURE: bloqueo estricto.
        if showMessages
...
def pbAddPokemon(*args)
  ...
  if ChallengeModes.had_first_encounter?(pkmn) && !ChallengeModes.on?(:GIFT_CLAUSE) && !ChallengeModes.counter_only?
  ...
  ChallengeModes.set_first_encounter(pkmn) if !ChallengeModes.on?(:GIFT_CLAUSE) && !ChallengeModes.counter_only?
  return ret
end
...
def pbAddPokemonSilent(*args)
  ...
  pkmn = args[0]; level = args[1]
  pkmn = Pokemon.new(pkmn, level) if !pkmn.is_a?(Pokemon)
  return false if ChallengeModes.had_first_encounter?(pkmn) && !ChallengeModes.on?(:GIFT_CLAUSE) && !ChallengeModes.counter_only?
...
def pbGenerateEgg(*args)
  ...
  if ChallengeModes.had_first_encounter?(args[0]) && !ChallengeModes.on?(:GIFT_CLAUSE) && !ChallengeModes.counter_only?
    rule_name = ChallengeModes.on?(:FIRST_CAPTURE) ? _INTL(ChallengeModes::RULES[:FIRST_CAPTURE][:display_name]) : _INTL(ChallengeModes::RULES[:ONE_CAPTURE][:display_name])
    pbMessage(_INTL("La regla \"{1}\" te impide obtener un Pokémon en un mapa en el que ya tuviste un encuentro!", rule_name))
    return false
  end
  ret = __challenge__pbGenerateEgg(*args)
  ChallengeModes.set_first_encounter(args[0]) if !ChallengeModes.on?(:GIFT_CLAUSE) && !ChallengeModes.counter_only?
  return ret
end
